...
import { Pencil, Eye, Trash2, Search, MapPin, FilterX, Download } from "lucide-react";

const API_URL = import.meta.env.VITE_API_URL;

// ── CSV Export Utility ─────────────────────────────────────────────────────────
const CSV_EXPORT_KEYS = ["custId", "name", "state", "district", "mandal"];

function downloadCSV(rows, filename = "customers.csv") {
  const escape = (v) => {
    const s = String(v ?? "");
    return s.includes(",") || s.includes('"') || s.includes("\n")
      ? `"${s.replace(/"/g, '""')}"` : s;
  };
  const header = CSV_EXPORT_KEYS.join(",");
  const body   = rows.map(r => CSV_EXPORT_KEYS.map(k => escape(r[k])).join(",")).join("\n");
...
export default function CustomersList() {
  ...
  const handleReset = () => {
    setState(""); setDistrict(""); setMandal(""); setSearchId(""); setSearchName("");
    setDistricts([]); setMandals([]);
  ...
  const handleExport = () => {
    if (!filtered.length) { alert("No customers to export."); return; }
    downloadCSV(filtered, `clients_export_${new Date().toISOString().slice(0, 10)}.csv`);
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 bg-slate-50 min-h-screen rounded-3xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={handleExport}
          className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 shadow-sm transition active:scale-95 self-start sm:self-auto">
          <Download className="w-4 h-4 text-emerald-500"/> Export CSV ({filtered.length})
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <input
            className={inputClass} placeholder="Enter Client ID..."
        ...
        <div className="flex gap-3 mt-5 border-t border-slate-100 pt-5 justify-end">
          <button onClick={handleReset}
            className="px-5 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-colors flex items-center gap-2 text-sm">
            <FilterX className="w-4 h-4"/> Reset
          </button>
          <button onClick={() => applyFilter()}
            className="px-6 py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-sm flex items-center gap-2 text-sm active:scale-95">
              ...
              {filtered.length ? (
                filtered.map((c) => (
                  <tr key={c.custId} className="hover:bg-indigo-50/30 transition-colors group">
                    <td className="p-4">
                      <span className="font-mono font-bold text-slate-600 bg-slate-50 rounded px-2 py-0.5 border border-slate-200 text-xs">{c.custId}</span>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => navigate(`/customers/view/${c.custId}`)}
                          className="p-2 rounded-xl bg-white border border-slate-200 hover:bg-indigo-50 text-indigo-600 shadow-sm transition">
                          <Eye className="w-4 h-4" />
                        </button>
                        <button onClick={() => navigate(`/customers/edit/${c.custId}`)}
                          className="p-2 rounded-xl bg-white border border-slate-200 hover:bg-amber-50 text-amber-600 shadow-sm transition">
                          <Pencil className="w-4 h-4" />
                        </button>
                        <button onClick={() => deleteCustomer(c.custId)}
                          className="p-2 rounded-xl bg-white border border-rose-100 hover:bg-rose-50 text-rose-500 shadow-sm transition">
